...
- Отдельный скрипт `db/migrate_to_partition.sql` для перевода `chat_messages` на партиционирование без триггеров.
...
Если таблица `chat_messages` уже содержит данные и её нужно перевести на партиционирование, выполните в DBeaver содержимое `db/migrate_to_partition.sql`:

1. Скрипт переименует старую таблицу в `chat_messages_old`.
2. Создаст партиционированную таблицу по `received_at` с теми же столбцами и индексацией.
3. Создаст недельные разделы c 1 декабря 2025 года на два года вперёд и `DEFAULT` для всего остального.
4. Перенесёт все строки в подходящие разделы.
5. Синхронизирует последовательность `id` и пересоздаст вьюху `v_last_messages`.

После проверки можно удалить `chat_messages_old` (последняя строка скрипта закомментирована, чтобы сделать это вручную).

### Откат с партиционированной таблицы на обычную
На случай, если понадобится вернуть обычную таблицу, можно выполнить `db/rollback_partition.sql` в той же сессии DBeaver:

1. Скрипт переименует партиционированную `chat_messages` в `chat_messages_partitioned`.
2. Создаст обычную таблицу с прежними ограничениями и индексом по `(channel, sent_at)`.
3. Перенесёт все данные обратно, синхронизирует последовательность `id` и пересоздаст вьюху `v_last_messages`.
4. По необходимости можно удалить старую партиционированную таблицу (закомментированная строка в конце).
